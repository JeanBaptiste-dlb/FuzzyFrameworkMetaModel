...
int main(){
	...
	ValueModel<float> tips(0);ValueModel<float> food(10);ValueModel<float> service(2);Expression<float>* t1 = f.newThen(f.newIs(&excellent, &service ),f.newIs(&delicious, &food ));
	Expression<float>* system = f.newDefuzz(t1,&tips);
