	...
	def checkAffectation(Affectation a) {
		if (a.value !== null) {
			if (a.value.equals("Gaussian")) {
				if (a.params.size !== 2) {
					error("Gaussian function requires 2 parameters : center:T and stddev:T",
			...
			if (a.value.equals("Ramp")) {
				...
				if (!a.params.get(2).value.equals("Up") && !a.params.get(2).value.equals("Down")) {
					error("The third parameter has to be a direction : Up or Down", Literals.AFFECTATION__VALUE);
					return;
				}
			}
			...
			if (a.value.equals("Trapeze")) {
				...
				if (!a.params.get(4).equals("CONCAVE") && a.params.get(4).equals("CONVEXE")) {
					error("Must be a concavity : CONCAVE or CONVEXE", Literals.AFFECTATION__PARAMS);
					return;
				}
			}
		}
